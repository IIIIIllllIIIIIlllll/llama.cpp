...
// RDNA3 WMMA version of the lightning indexer kernel, ported from the NVIDIA kernel above.
// Uses the ggml_cuda_mma tile abstraction (mma.cuh) instead of nvcuda::wmma.
// A single RDNA3 WMMA instruction computes a 16x16x16 tile, so each inner loop iteration
// processes 16 heads (instead of 8 for the NVIDIA m8n32k16 kernel) and 2 accumulators
// are needed to cover K_VECS_PER_BLOCK=32 K vectors. As in the NVIDIA kernel each of the
// 8 warps computes a 16 dim slice of the 128 dim embd dot product and the partial results
// are reduced via qk_shared.
// Note that on RDNA3 matrix C is "column-major": for mma(C, A, B) the i index of C
// (T_C::get_i) runs over the rows of B and the j index (T_C::get_j) over the rows of A,
// so the K fragment is passed as A and the Q fragment as B to get C[head][kv].

template <int WARPS_PER_BLOCK, int K_VECS_PER_BLOCK, int64_t N_EMBD, int64_t N_HEAD, ggml_type TYPE_K>
static __global__ void lightning_indexer_kernel_mma_hip(
        const float * Q, const char * K, const float * W, const half * M, float * dst,
        int64_t n_stream, int64_t n_batch, int64_t n_kv,
        size_t nb1, size_t nb2, size_t nb3,
        size_t nbq1, size_t nbq2, size_t nbq3,
    ...
    static_assert(N_EMBD % (WARPS_PER_BLOCK*K_EMBD_PER_INNER_LOOP) == 0, "bad N_EMBD");
    static_assert(N_HEAD % HEADS_PER_INNER_LOOP == 0, "bad N_HEAD");
    static_assert(K_VECS_PER_BLOCK == 2*HEADS_PER_INNER_LOOP, "bad K_VECS_PER_BLOCK");

    typedef ggml_cuda_mma::tile<16,  8, half2, ggml_cuda_mma::DATA_LAYOUT_I_MAJOR_MIRRORED> tile_ab;
    typedef ggml_cuda_mma::tile<16, 16, float, ggml_cuda_mma::DATA_LAYOUT_I_MAJOR>          tile_c;
    ...
    __shared__ float w_shared[N_HEAD];
    // int4 storage guarantees the 16 byte alignment required by load_ldmatrix,
    // each row holds N_EMBD_PADDED half values (N_EMBD_PADDED/2 half2 stride)
    __shared__ int4  q_shared_h[HEADS_PER_INNER_LOOP][N_EMBD_PADDED / 8];

    if (tid < N_HEAD) {
        w_shared[tid] = w_base[tid];
    }

    // total number of half4 (int2) elements in HEADS_PER_INNER_LOOP x N_EMBD Q tile
    constexpr int N_Q_TILE = HEADS_PER_INNER_LOOP * (N_EMBD / 4);
    // number of registers needed in each thread to store Q tile in thread block
    constexpr int N_Q_NEXT = (N_Q_TILE + THREADS_PER_BLOCK - 1) / THREADS_PER_BLOCK;
...
#pragma unroll
    for (int i_q = tid; i_q < N_Q_TILE; i_q += THREADS_PER_BLOCK) {
        ...
        const int i_embd = i_q % (N_EMBD / 4);
        const float4 q = *(const float4 *) (q_base + i_head*nbq1 + i_embd*sizeof(float4));
        half2 * q_dst = (half2 *) &((int2 *) q_shared_h[i_head])[i_embd];
        q_dst[0] = __float22half2_rn(make_float2(q.x, q.y));
        q_dst[1] = __float22half2_rn(make_float2(q.z, q.w));
    }

    // phase 2 - load (and dequantize if needed) K to shared mem

    __shared__ int4 k_shared_h[K_VECS_PER_BLOCK][N_EMBD_PADDED / 8];
...
#pragma unroll
        for (int i_k = tid; i_k < n_k; i_k += THREADS_PER_BLOCK) {
            ...
            if (i_kv < n_kv) {
                const void * k_base = (const void *) ((const char *) K + i_kv*nbk2 + i_stream*nbk3);
                dequantize_k(k_base, (half2 *) &((int2 *) k_shared_h[i_k_vec])[i_embd], i_embd * 4);
            } else {
                ((int2 *) k_shared_h[i_k_vec])[i_embd] = make_int2(0, 0);
            }
        }
    ...
    // load K fragments, one 16x16 tile per 16 K vectors (K vectors are the rows of the tile)
    tile_ab frag_k[2];
    ggml_cuda_mma::load_ldmatrix(frag_k[0], (const half2 *) k_shared_h[0]                    + i_warp*(K_EMBD_PER_INNER_LOOP/2), N_EMBD_PADDED/2);
    ggml_cuda_mma::load_ldmatrix(frag_k[1], (const half2 *) k_shared_h[HEADS_PER_INNER_LOOP] + i_warp*(K_EMBD_PER_INNER_LOOP/2), N_EMBD_PADDED/2);
    ...
    for (int i_head_0 = 0; i_head_0 < N_HEAD; i_head_0 += HEADS_PER_INNER_LOOP) {
        ...
        // accumulators, one per 16 K vectors, initialized to zero
        tile_c frag_acc[2];

        // load Q fragment
        tile_ab frag_q;
        ggml_cuda_mma::load_ldmatrix(frag_q, (const half2 *) q_shared_h[0] + i_warp*(K_EMBD_PER_INNER_LOOP/2), N_EMBD_PADDED/2);
...
#pragma unroll
        ...
        // perform matrix multiplication
        ggml_cuda_mma::mma(frag_acc[0], frag_k[0], frag_q);
        ggml_cuda_mma::mma(frag_acc[1], frag_k[1], frag_q);

        // store the per-warp partial results to shared memory,
        // matrix C is column-major on RDNA3: i = head (rows of the B fragment), j = K vector
#pragma unroll
        for (int l = 0; l < tile_c::ne; ++l) {
            const int h = tile_c::get_i(l);
            const int k = tile_c::get_j(l);
            qk_shared[i_warp][h][k                          ] = frag_acc[0].x[l];
            qk_shared[i_warp][h][k + HEADS_PER_INNER_LOOP   ] = frag_acc[1].x[l];
        }
...
#pragma unroll
            for (int i_q = tid, i_q_next = 0; i_q < N_Q_TILE; i_q += THREADS_PER_BLOCK) {
                ...
                const int i_embd = i_q % (N_EMBD / 4);
                half2 * q_dst = (half2 *) &((int2 *) q_shared_h[i_head])[i_embd];
                q_dst[0] = __float22half2_rn(make_float2(q_next[i_q_next].x, q_next[i_q_next].y));
                q_dst[1] = __float22half2_rn(make_float2(q_next[i_q_next].z, q_next[i_q_next].w));
                ++i_q_next;
            }
        }

        // accumulate QK multiplication results from all block warps
        // (there are 256 threads in block and 16*32 = 512 matmul outputs, 2 per thread;
        //  each (h, k) pair is owned by exactly one thread so no extra sync is needed
        //  before reusing qk_shared[0] for the partial results)
        // TODO it will break if WARP_SIZE is not 32
#pragma unroll
        for (int r = 0; r < (HEADS_PER_INNER_LOOP*K_VECS_PER_BLOCK)/THREADS_PER_BLOCK; ++r) {
            const int hk = tid + r*THREADS_PER_BLOCK;
            const int h = hk / K_VECS_PER_BLOCK;
            const int k = hk % K_VECS_PER_BLOCK;
            const float w_val = w_shared[i_head_0 + h];
...
#else // defined(AMD_WMMA_AVAILABLE) && defined(RDNA3)
    GGML_UNUSED_VARS(Q, K, W, M, dst,
        n_stream, n_batch, n_kv,
        nb1, nb2, nb3,
        nbq1, nbq2, nbq3,
...
void ggml_cuda_lightning_indexer(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (n_embd == 128 && n_head == 64) {
#if defined(GGML_USE_HIP)
        // The HIP WMMA kernel implements RDNA3 only, RDNA4 falls back to the vector kernel for now.
        if (GGML_CUDA_CC_IS_RDNA3(cc) && k->type != GGML_TYPE_F32 && k->type != GGML_TYPE_BF16) {
            // use wmma kernel
            ...
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_mma_hip, 128, 64, k, GGML_TYPE_F16)
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_mma_hip, 128, 64, k, GGML_TYPE_Q4_0)
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_mma_hip, 128, 64, k, GGML_TYPE_Q4_1)
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_mma_hip, 128, 64, k, GGML_TYPE_Q5_0)
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_mma_hip, 128, 64, k, GGML_TYPE_Q5_1)
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_mma_hip, 128, 64, k, GGML_TYPE_Q8_0)
        ...
        if (GGML_CUDA_CC_IS_RDNA3(cc) && k->type != GGML_TYPE_F32 && k->type != GGML_TYPE_BF16) {
            ...
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_mma_hip, 128, 32, k, GGML_TYPE_F16)
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_mma_hip, 128, 32, k, GGML_TYPE_Q4_0)
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_mma_hip, 128, 32, k, GGML_TYPE_Q4_1)
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_mma_hip, 128, 32, k, GGML_TYPE_Q5_0)
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_mma_hip, 128, 32, k, GGML_TYPE_Q5_1)
            LIGHTNING_INDEXER_CASE(lightning_indexer_kernel_mma_hip, 128, 32, k, GGML_TYPE_Q8_0)
            GGML_ABORT("fatal error");
        } else {
#elif !defined(GGML_USE_MUSA)
        if (GGML_CUDA_CC_IS_NVIDIA(cc) && turing_mma_available(cc) && k->type != GGML_TYPE_F32 && k->type != GGML_TYPE_BF16) {
